...
T = 0.1（尖锐）:  [0.01, 0.01, 0.96, 0.01, 0.01] → 几乎确定选第3个
T = 1.0（原始）:  [0.10, 0.05, 0.40, 0.15, 0.30] → 按原始概率
T = 2.0（平坦）:  [0.18, 0.15, 0.25, 0.20, 0.22] → 更接近均匀随机
...
| 速度 | 最慢（需要维护 k 条候选，每步 k 次 forward pass） |
| 显存 | k 倍的 decode 开销 |
...
```
以 Llama-3-8B 在 H100 上为例（batch=32）：
...
Beam Search (k=3)    | 2,200       | 0.34x   | 需要 3 次 forward pass
Beam Search (k=5)    | 1,300       | 0.20x   | 需要 5 次 forward pass
```
