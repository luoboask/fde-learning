...
```mermaid
flowchart TD
    L1["L1: 基础提示<br/>简单指令，如'帮我写一段代码'"] --> L2
    L2["L2: 结构化提示<br/>角色 + 任务 + 约束 + 示例"] --> L3
    L3["L3: 思维链 + 工具调用<br/>CoT / ReAct / Function Calling"] --> L4
    L4["L4: 系统化 Prompt 架构<br/>模板引擎 + 版本管理 + 自动评估"]
...
sequenceDiagram
    ...
    Note over M,S: 每一步的中间结果都进入 KV Cache<br/>成为下一步的 context，形成推理链
```
...
```mermaid
flowchart LR
    A[用户请求] --> B[Prompt 组装<br/>模板 + 上下文 + 历史]
    B --> C[Tokenize<br/>计算 token 数]
    C --> D{超过 max_tokens?}
    D -->|是| E[上下文截断<br/>或报错]
    D -->|否| F[LLM 推理]
    F --> G[输出解析<br/>JSON/结构化提取]
    G --> H[返回给用户]
