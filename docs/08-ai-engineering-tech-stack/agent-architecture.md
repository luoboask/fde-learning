...
```mermaid
graph TD
    A[用户目标<br/>"分析这家公司去年的财务数据"] --> B[Agent Core]

    subgraph "Agent 核心循环"
        B --> C[Perceive 感知<br/>理解当前状态]
        C --> D[Plan 规划<br/>拆解任务、制定策略]
        D --> E[Act 行动<br/>调用工具、执行操作]
        E --> F[Observe 观察<br/>获取执行结果]
        F --> G{目标达成?}
        G -->|否| C
        G -->|是| H[返回结果]
    end
...
```mermaid
flowchart LR
    V1["v1 Chat<br/>纯对话<br/>2022"] --> V2
    V2["v2 Plugin<br/>工具调用<br/>2023"] --> V3
    V3["v3 Agent<br/>自主规划<br/>2024"] --> V4
    V4["v4 Multi-Agent<br/>协作系统<br/>2025"] --> V5
    V5["v5 Autonomous<br/>全自主 Agent<br/>2026"]
...
```mermaid
graph TD
    A["__start__"] --> B[agent_node<br/>LLM 决策]
    B --> C{选择工具}
    C -->|search| D[search_node<br/>搜索工具]
    C -->|code| E[code_node<br/>代码执行]
    C -->|calculator| F[calc_node<br/>计算器]
    C -->|done| G["__end__"]
    D --> B
    E --> B
    F --> B
...
```mermaid
graph TD
    A[记忆系统] --> B[短期记忆<br/>Session Memory]
    A --> C[工作记忆<br/>Working Memory]
    A --> D[长期记忆<br/>Long-term Memory]
    A --> E[情景记忆<br/>Episodic Memory]

    B --> B1[当前对话上下文<br/>生命周期: 单次会话]
    C --> C1[当前任务的中间状态<br/>生命周期: 单个任务]
    D --> D1[用户画像、偏好<br/>存储: 向量数据库]
    E --> E1[历史交互的总结<br/>存储: 摘要+索引]
...
flowchart TB
    ...
    subgraph "Agent 服务"
        G[Gateway<br/>请求路由]
        S[Session Manager<br/>会话管理]
        A[Agent Core<br/>LangGraph/CrewAI]
        M[Memory Service<br/>记忆管理]
        T[Tool Registry<br/>工具注册]
    end
